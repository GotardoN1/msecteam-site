...
function resizePlayerPhoto(file: File) {
  return new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => { const image = new Image(); image.onload = () => { const size = 900; const scale = Math.min(1, size / Math.max(image.width, image.height)); const canvas = document.createElement("canvas"); canvas.width = Math.round(image.width * scale); canvas.height = Math.round(image.height * scale); canvas.getContext("2d")?.drawImage(image, 0, 0, canvas.width, canvas.height); resolve(canvas.toDataURL("image/jpeg", .82)); }; image.onerror = reject; image.src = String(reader.result); }; reader.onerror = reject; reader.readAsDataURL(file); });
}
...
function TeamEditor({ content, update }: { content: SiteContent; update: <K extends keyof SiteContent>(key: K, value: SiteContent[K]) => void }) {
  ...
  const editPlayer = (index: number, patch: Partial<SiteContent["players"][number]>) => { const next = [...players]; next[index] = { ...next[index], ...patch }; update("players", next); };
  const handlePhoto = async (index: number, file?: File) => { if (!file) return; if (!file.type.startsWith("image/")) return window.alert("Escolha um arquivo de imagem."); try { editPlayer(index, { image: await resizePlayerPhoto(file) }); } catch { window.alert("Não foi possível carregar esta foto."); } };
  return <div className="master-list">{players.map((player, index) => <div className="master-card team-edit-card" key={`${player.tag}-${index}`}><div className="master-card-number">0{index + 1}</div><div className="team-edit-layout"><label className="team-photo-picker"><img src={player.image || assetPath("msec-logo.png")} alt={`Foto de ${player.tag}`} /><span>Trocar foto<input type="file" accept="image/*" onChange={(event) => handlePhoto(index, event.target.files?.[0])} /></span></label><div className="field-grid compact"><Field label="Nome / tag" value={player.tag} onChange={(value) => editPlayer(index, { tag: value })} /><Field label="Função" value={player.role} onChange={(value) => editPlayer(index, { role: value })} /><Field label="Nome real" value={player.realName || ""} onChange={(value) => editPlayer(index, { realName: value })} /><Field label="Rede social" value={player.social || ""} onChange={(value) => editPlayer(index, { social: value })} /></div></div></div>)}<button className="master-add" onClick={() => update("players", [...players, { tag: "NOVO", role: "PLAYER", number: String(players.length + 1).padStart(2, "0"), accent: "#b9d79d", image: assetPath("msec-logo.png") }])}>+ Adicionar jogador</button></div>;
}
